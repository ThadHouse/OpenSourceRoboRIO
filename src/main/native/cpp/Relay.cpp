...
void HAL_SetRelay(HAL_RelayHandle relayPortHandle, HAL_Bool on,
  ...
  uint8_t relays = 0;
  if (port->fwd) {
    *status = NiFpga_ReadU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Forward_Values, &relays);
  } else {
    *status = NiFpga_ReadU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Reverse_Values, &relays);
  }
  ...
  if (port->fwd) {
    *status = NiFpga_WriteU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Forward_Values, relays);
  } else {
    *status = NiFpga_WriteU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Reverse_Values, relays);
  }
}
