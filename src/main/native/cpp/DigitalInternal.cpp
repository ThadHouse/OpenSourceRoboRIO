...
void initializeDigital(int32_t* status) {
  ...
  NiFpga_WriteU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Forward_Values, 0);
  NiFpga_WriteU8(FPGASession, NiFpga_OpenSourceRIO_ControlU8_Relay_Reverse_Values, 0);

  // Disable all PWMs

  constexpr double loopTime = 0.001; // loop time is 1us TODO return this from FPGA

  constexpr uint16_t period = static_cast<uint16_t>(kDefaultPwmPeriod / loopTime + 0.5);

  constexpr uint16_t minHigh = static_cast<uint16_t>((kDefaultPwmCenter - kDefaultPwmStepsDown * loopTime) / loopTime + 0.5);

  // Set period
  NiFpga_WriteU16(FPGASession, NiFpga_OpenSourceRIO_ControlU16_PWM_Period, period);
  NiFpga_WriteU16(FPGASession, NiFpga_OpenSourceRIO_ControlU16_PWM_MinHigh, minHigh);

  for (int i = 0; i < 10; i++) {
    NiFpga_WriteU32(FPGASession, NiFpga_OpenSourceRIO_ControlU32_PWM_SetValue, (static_cast<uint32_t>(i) << 16) | kPwmDisabled);
    NiFpga_WriteU16(FPGASession, NiFpga_OpenSourceRIO_ControlU16_PWM_SetPeriodScale, (static_cast<uint32_t>(i) << 8) | 3);
  }
